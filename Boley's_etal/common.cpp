...
void checkArgs(int argc, char *argv[]){
  if(argc<7){
    fprintf(stderr, "usage: %s (pattern_file)(graph_file)(phenotype_file)(population_file)(theta)(alpha)[(param1)(value1)(param2)...]\n\n", argv[0]);
...
void readPhenotype(Param P, Tool T){
  ...
  while (fgets(str, LINE_MAX - 1, in) != NULL) {
      ...
      if(atoi(str_phenotype)) T->phenotype.set(i);
  }
  delete[] str;
  delete[] str_iid;
...
int getClosure(Param P, Tool T, Graph G, BFSTool B, Solution *S, BanList *Ban, Vertex V, itemset Item, itemset* result) {
    ...
    if (numItems < P->theta) return 0;
    else if (Ban->bit[V->id]) return 0;
    queue<int> Q;
    int top, nextV;
    Q.push(V->id);
    auto end_itr = S->seq.end();
    for (auto itr = S->seq.begin(); itr != end_itr; ++itr) {
        if (*itr != -1) Q.push(G->V[*itr]->id);
    }
    unsigned int marker = prepareBFS(B, G->n, S->seq);

    B->marker = B->marker + 1;//mark of v which was push on Q but not checked about C
    while (!Q.empty()) {
        top = Q.front();
...
vector<int> adjList(BanList *Ban,  Graph G, OwnStack S) {
    ...
    for (auto itr = S->seq.begin(); itr != end; ++itr) {
        if (*itr == -1) continue;
        for (auto itr2 = G->V[*itr]->A.begin(); itr2 != G->V[*itr]->A.end();itr2++) {
            id = *itr2;
            if (check[id]||S->bit[id] || Ban->bit[id]) continue;
...
double Pcmh(Param P, Tool T, Graph G, OwnStack S){
	...
	I.set();
	 ...
	 for (int i = 0; i < T->numPop;i++){
		 ...
		 denom += (1.0*T->n1[i]*T->n2[i]*xSj*(T->n[i]-xSj))/((T->n[i]-1.0)* T->n[i]*T->n[i]);
	 }
