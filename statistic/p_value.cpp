...
double _Stat::p_value(OwnStack S) {
    ...
    for (int j = 0; j < this->J; ++j) {
        ...
        asj = (double)((~(this->I_buffer)) & this->tool->population[j] & this->tool->phenotype).count();
...
double _Stat::envelope(OwnStack S) {
    ...
    for (int j = 0; j < this->J; ++j) {
        ...
        this->betaLs_buffer[j] = std::make_tuple(n1sj * std::max(xsj, n2sj) / nsj_2, j);
        this->betaRs_buffer[j] = std::make_tuple(n2sj * std::max(xsj, n1sj) / nsj_2, j);
    }
    ...
    for (int j = 0; j < this->J; ++j) {
        ...
        this->xStarsL_buffer[piL] = std::max(this->xs_buffer[piL], (double)(this->tool->n2[piL]));
        env = std::max(env, this->minimal_p_value_inner(this->xStarsL_buffer));
        
        this->xStarsR_buffer[piR] = std::max(this->xs_buffer[piL], (double)(this->tool->n1[piL]));
